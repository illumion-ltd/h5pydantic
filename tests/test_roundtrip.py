...
def type_and_value_st(draw, recursive:bool):
    dtype = draw(st.sampled_from([H5Integer32, H5Integer64, float, str] + [H5Dataset, H5Group] * recursive))
...
def dataset_st(draw):
    ...
    dtype = create_model(classname, __base__=H5Dataset, __cls_kwargs__={"shape": shape, "dtype": H5Integer32}, **d)
    value = dtype()
